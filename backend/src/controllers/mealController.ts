...
export async function getMealById(req: AuthRequest, res: Response) {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Failed to fetch meal' })
  }
}
...
export async function getDailyNutritionSummary(req: AuthRequest, res: Response) {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Failed to fetch daily nutrition' })
  }
}
